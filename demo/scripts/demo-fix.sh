...
log_info "Running remediation workflow for demo violations with provider=${DEMO_PROVIDER}"
if ! ansible-playbook "${VAGRANT_CWD}/../playbooks/scenario-b-drift.yml" -i "${INVENTORY_FILE}" --tags fix; then
  log_error "Failed to remediate demo violations"
  exit 1
fi

log_success "Demo violations remediated"
